<script>
    $(document).ready(()=>{
        $('.modal_parténaire').click(function (e){
            e.preventDefault()
            $('#modal_parténaire').modal('show')
        })
    })
</script>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#AddPartenaires').click(function (e){
            e.preventDefault()
            $("#AddPartenaires").empty().append('<i class="fa fa-spinner fa-spin"></i> En cours d\'envoi...');
            $("#AddPartenaires").prop('disabled', false);
            var type = $('input[name="titre"]').val();
            ...
            if(image_empty.length ==''){
                ...
                $("#AddPartenaires").empty().append('<i class="fa fa-repeat"></i> Réessayer encore');
                $("#AddPartenaires").prop('disabled', false);
            }else{
                if(image){
                ...
                formData.append("titre",type);
                formData.append("image",image);
                $.ajax({
                    url: '{{route('admin.storePartenaire')}}',
                    method: 'POST',
                    data: formData,
                    contentType: false,
                    cache: false,
    ...
    function deletePartenaires(ID){
        var csrf_token=$('meta[name="csrf_token"]').attr('content');
        $.ajax({
            type:'POST',
            url:"{{route('admin.deletePartenaire')}}",
            data:{
                ID:ID,
                '_token' :csrf_token,
            },
